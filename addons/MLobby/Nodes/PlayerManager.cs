...
public partial class PlayerManager : MultiplayerSpawner
{
	...
	{
		// Ask the validate peer for its player info
		RpcId(peerID, nameof(RPCPlayerInfoRequested));
	}
	[Rpc(MultiplayerApi.RpcMode.Authority, CallLocal = true, TransferMode = MultiplayerPeer.TransferModeEnum.Reliable)]
	private void RPCPlayerInfoRequested()
	{
		// Client game instance send back its name
		string pName = (MSettings.Settings.GetCachedSettings("GameConfigSettings") as GameConfigSettings).playerName;
		RpcId(1, nameof(RPCReceivePlayerInfoFromPeer), pName);
	}
	[Rpc(MultiplayerApi.RpcMode.AnyPeer, CallLocal = true, TransferMode = MultiplayerPeer.TransferModeEnum.Reliable)]
	private void RPCReceivePlayerInfoFromPeer(string pName)
	{
		if(!Multiplayer.IsServer()){ return; }
		int peerID = Multiplayer.GetRemoteSenderId() == 0 ? 1 : Multiplayer.GetRemoteSenderId();
		if(GetPlayer(peerID, out MLobbyPlayer player))
		{
			MLogging.MLog.LogError($"PlayerManager::RPCReceivePlayerInfoFromPeer({pName}) from peer [{peerID}] but that player data was already created!");
			return;
		}
		pName = ModifyNameForDupes(peerID, pName);
		Godot.Collections.Dictionary<string, Variant> SpawnPlayerDataObjectArgs = new Godot.Collections.Dictionary<string, Variant>()
		{
			{"peerID", peerID },
			{"playerName", pName == string.Empty ? $"Player-{players.Count + 1}" : pName }
		};
		players.Add(Spawn(SpawnPlayerDataObjectArgs) as MLobbyPlayer);
		Rpc(nameof(RPCRaisePlayersChanged));
	}

	private string ModifyNameForDupes(int peerID, string pName)
	{
		// Check for duplicate name
		int dupes = players.FindAll(p=>p.PeerID != peerID && p.PlayerName.Contains(pName)).Count;
		if(dupes > 0) { pName += $"({dupes+1})"; }
		return pName;
	}
	...
	internal void SetNameOnPlayer(long peerID, string newName)
	{
		if (GetPlayer(peerID, out MLobbyPlayer player))
		{
			newName = ModifyNameForDupes((int)peerID, newName);
