...
public partial class HUDSystem : CanvasLayer
{
    ...
    public static void ToggleElement(string elementName, bool syncMouse = false)
    {
        ...
        {
            if (Instance.HUDElements[elementName] is IHUDElement e)
            {
                e.Toggle();
                return;
            }
    ...
    internal static bool IsElementActive(string name)
    {
        if (Instance.HUDElements.ContainsKey(name))
        {
            return Instance.HUDElements[name].Visible;
        }
        return false;
    }
}// EOF CLASS