...
public partial class PlayerAvatar : RigidBody3D, ITargetable
{
	...
	[Export] Curve accelerationCurve;
	...
	public override void _PhysicsProcess(double delta)
	{
		...
		if (inLeftStick != Vector3.Zero)
		{
			...
			ApplyForce(inLeftStick * Mass * GetAcceleration());
		}
		else
		{
	...
	{
		float dot = LinearVelocity == Vector3.Zero ? 1.0f : inLeftStick.Dot(LinearVelocity);
		if (dot < 0.2f)
		{
			//GD.Print($"Breaking!  DOT[{inLeftStick.Dot(LinearVelocity)}] inLeftStick[{inLeftStick.Length()}] LinearVelocity[{LinearVelocity.Length()}]");
			LinearVelocity *= 0.5f;
		}
		float normalizedAccelModifier = LinearVelocity.Length() / moveSpeed;
		float accel = acceleration * accelerationCurve.Sample(normalizedAccelModifier);
